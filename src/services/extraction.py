...
def _normalize_llm_content(content: str, source_text: str) -> str:
    ...
    if ("is running" in lower or "running" in lower) and "times a week" in lower:
        text = re.sub(
            r"(?i)(the\s+)?user is running\s+(\d+)\s+times a week",
            r"User runs \2 times a week",
            text,
        )
    if (
        "runs" in text.lower()
        and "times a week" in text.lower()
        and not text.lower().startswith("user ")
    ):
        text = "User " + text
...
def extract_from_transcript(request: TranscriptRequest) -> ExtractionResult:
    ...
    if memories:
        ...
        if existing_memories_checked > 0:
            summary += (
                f" Checked {existing_memories_checked} existing memories for context."
            )
