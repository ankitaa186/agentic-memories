...
def node_init(state: IngestionState) -> IngestionState:
    ...
    logger.info(
        "[graph.init] user_id=%s history_length=%s",
        state.get("user_id"),
...
def node_worthiness(state: IngestionState) -> IngestionState:
    ...
    _span = start_span(
        "worthiness_check", input={"history_count": len(state.get("history", []))}
    )
    ...
    state["worthy"] = worthy
    state["metrics"]["worthiness_check_ms"] = int(
        (time.perf_counter() - state["t_start"]) * 1000
    )
...
def node_extract(state: IngestionState) -> IngestionState:
    ...
    _span = start_span(
        "memory_extraction",
        input={"user_id": user_id, "existing_memories_count": len(existing_memories)},
    )
    ...
    # Process ALL messages to capture initial profile information
    payload = {"history": history_dicts, "existing_memories_context": existing_context}
    ...
    state["metrics"]["extraction_ms"] = int(
        (time.perf_counter() - state["t_start"]) * 1000
    )
...
def node_classify_and_enrich(state: IngestionState) -> IngestionState:
    ...
    _span = start_span(
        ...
        input={"items_count": len(state.get("extracted_items", []))},
    )
    ...
    for item in items:
        ...
        if emotional_context:
            ...
            sentiment = {
                ...
                "dominant_emotion": emotional_context.get(
                    "dominant_emotion", "neutral"
                ),
                "importance": float(emotional_context.get("importance", 0.5)),
            }
            classification["sentiment"] = sentiment
            classification["is_emotional"] = (
                layer == "emotional" or emotional_context.get("importance", 0) > 0.7
            )
            logger.debug(
                "[graph.classify] Using V3 emotional_context: valence=%.2f arousal=%.2f emotion=%s",
                sentiment["valence"],
                sentiment["arousal"],
            ...
            classification["sentiment"] = sentiment
            classification["is_emotional"] = (
                sentiment.get("has_emotional_content", False) if sentiment else False
            )
        else:
    ...
    state["metrics"]["classification_ms"] = int(
        (time.perf_counter() - state["t_start"]) * 1000
    )

    logger.info(
        "[graph.classify] user_id=%s classified=%s emotional=%s episodic=%s procedural=%s",
        state.get("user_id"),
        len(classifications),
...
def _merge_request_metadata(
    base_metadata: Dict[str, Any], request: Optional[TranscriptRequest]
) -> None:
    ...
    if not isinstance(request_metadata, dict):
        logger.warning(
            "[graph.build_memories] request.metadata is not a dict: %s",
    ...
    for key, value in request_meta_copy.items():
        if (
            ...
            and isinstance(base_metadata[key], dict)
...
def node_build_memories(state: IngestionState) -> IngestionState:
    ...
    state["metrics"]["build_memories_ms"] = int(
        (time.perf_counter() - state["t_start"]) * 1000
    )

    logger.info(
        "[graph.build_memories] user_id=%s memories_built=%s",
        state.get("user_id"),
        len(memories),
    )
...
def node_dedup_check(state: IngestionState) -> IngestionState:
    ...
    _span = start_span(
        "dedup_check", input={"memories_count": len(state.get("memories", []))}
    )
    ...
    try:
        ...
        for idx, memory in enumerate(memories):
            ...
            try:
                ...
                if (
                    hash_results
                    and hash_results.get("ids")
                    and len(hash_results["ids"]) > 0
                ):
                    is_duplicate = True
                ...
                logger.debug(
                    "[graph.dedup_check.hash_query_error] user_id=%s error=%s",
                    user_id,
                    exc,
                )
            ...
            if not is_duplicate and memory.embedding:
                try:
                    ...
                    logger.warning(
                        "[graph.dedup_check.query_error] user_id=%s error=%s",
                        user_id,
                        exc,
                    )
            ...
            if is_duplicate:
                ...
                logger.info(
                    "[graph.dedup_check.duplicate] user_id=%s match=%s hash=%s content=%s",
                    user_id,
                    match_type,
                    content_hash[:12],
                    memory.content[:80],
        ...
        logger.warning(
            "[graph.dedup_check.error] user_id=%s error=%s — passing all memories through",
            user_id,
            exc,
        )
    ...
    if classifications:
        state["classifications"] = [
            classifications[i] for i in keep_indices if i < len(classifications)
        ]

    state["metrics"]["duplicates_avoided"] = duplicates_avoided

    logger.info(
        "[graph.dedup_check] user_id=%s input=%s output=%s duplicates_avoided=%s (hash=%s semantic=%s)",
        user_id,
        len(memories),
        len(state["memories"]),
        duplicates_avoided,
...
def node_extract_profile(state: IngestionState) -> IngestionState:
    ...
    _span = start_span(
        "extract_profile", input={"memories_count": len(state.get("memories", []))}
    )
    ...
    try:
        ...
        logger.info(
            "[graph.extract_profile] user_id=%s extractions=%s",
            user_id,
            len(extractions),
        )
...
def node_store_profile(state: IngestionState) -> IngestionState:
    ...
    _span = start_span(
        "store_profile",
        input={"extractions_count": len(state.get("profile_extractions", []))},
    )
    ...
    try:
        ...
        logger.info(
            "[graph.store_profile] user_id=%s fields_stored=%s", user_id, fields_updated
        )
...
def node_store_chromadb(state: IngestionState) -> IngestionState:
    ...
    _span = start_span(
        "store_chromadb", input={"memories_count": len(state.get("memories", []))}
    )
        ...
        for i, (memory, classification) in enumerate(zip(memories, classifications)):
            ...
            if emotional_context:
                emotional_valence = float(
                    emotional_context.get("valence", emotional_valence)
                )
                emotional_arousal = float(
                    emotional_context.get("arousal", emotional_arousal)
                )
                importance = float(emotional_context.get("importance", importance))

            if sentiment or emotional_context:
                logger.debug(
                    "[graph.episodic] V3 emotional data: valence=%.2f arousal=%.2f importance=%.2f emotion=%s",
                    emotional_valence,
                    emotional_arousal,
                    importance,
                    sentiment.get("dominant_emotion")
                    or emotional_context.get("dominant_emotion", "unknown"),
                )
    ...
    try:
        ...
        for i, (memory, classification) in enumerate(zip(memories, classifications)):
            ...
            service.upsert_holding_from_memory(
                user_id=user_id, portfolio_metadata=portfolio_meta, memory_id=memory_id
            )
            stored_count += 1
...
def node_summarize_storage(state: IngestionState) -> IngestionState:
    ...
    logger.info(
        "[graph.summarize] user_id=%s total_stored=%s breakdown=%s",
        state.get("user_id"),
        total_stored,
        storage_results,
    )
...
def node_finalize(state: IngestionState) -> IngestionState:
    ...
    logger.info(
        "[graph.finalize] user_id=%s total_ms=%s storage_results=%s errors=%d",
        state.get("user_id"),
        total_ms,
...
def _is_episodic(item: Dict[str, Any], tags: List[str]) -> bool:
    ...
    # Debug logging
    logger.debug(
        "[_is_episodic] tags=%s layer=%s type=%s has_event_tag=%s result=%s",
        tags,
        layer,
        mtype,
        has_event_tag,
...
def _is_procedural(item: Dict[str, Any], tags: List[str]) -> bool:
    ...
    # Debug logging
    logger.debug(
        "[_is_procedural] tags=%s has_skill_tag=%s has_learning_journal=%s result=%s",
        tags,
        has_skill_tag,
        has_learning_journal,
        is_procedural,
    )
...
def _analyze_sentiment_llm(content: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(
            "[sentiment_analysis] Failed for content: %s | error=%s", content[:50], e
        )
        return None
...
def node_finalize_early(state: IngestionState) -> IngestionState:
    ...
    logger.info(
        "[graph.finalize_early] user_id=%s reason=%s",
        state.get("user_id"),
...
def run_unified_ingestion(request: TranscriptRequest) -> Dict[str, Any]:
    ...
    if is_langfuse_enabled():
        try:
            ...
            logger.info(
                "[unified_graph] Starting with Langfuse trace_id=%s for user_id=%s",
                trace_id,
                request.user_id,
            )
            ...
            logger.warning(
                "[unified_graph] LangChain not available for Langfuse integration: %s. Install langchain-core.",
                e,
            )
            ...
            logger.error(
                "[unified_graph] Langfuse callback failed: %s", e, exc_info=True
            )
            # Fallback to running without tracing
            final_state: Dict[str, Any] = compiled_graph.invoke(initial_state)
    else:
    ...
    logger.info(
        "[unified_graph] completed user_id=%s memories=%s total_ms=%s",
        request.user_id,
