...
def start_trace(name: str, user_id: str, metadata: Optional[Dict[str, Any]] = None) -> Optional[Any]:
	...
	try:
		...
		logger.info("[tracing] Started trace: name=%s user_id=%s trace_id=%s", name, user_id, trace_id)
		return root_span
...
def end_span(span: Optional[Any] = None, output: Optional[Dict[str, Any]] = None, level: str = "DEFAULT") -> None:
