...
def run_test_suite(suite_name: str, fixture_file: str, prompt: Any) -> Dict[str, Any]:
    ...
    overall_metrics = evaluate_extraction_comprehensive(
        all_results, include_token_metrics=True
    )
    ...
    for category, results in category_results.items():
        if len(results) >= 2:  # Only calculate if enough samples
            category_metrics[category] = evaluate_extraction_comprehensive(
                results, include_token_metrics=False
            )
...
def print_suite_summary(suite_result: Dict[str, Any]) -> None:
    ...
    if category_metrics:
        ...
        for category, cat_metrics in sorted_categories[:5]:
            ...
            print(
                f"  • {category:30s}  F1: {f1:.3f}  P: {precision:.3f}  R: {recall:.3f}"
            )
...
def compare_suites(suite_results: List[Dict[str, Any]]) -> None:
    ...
    print(
        f"{'Suite':<25} {'Tests':>8} {'F1':>8} {'Precision':>10} {'Recall':>8} {'Cost':>10}"
    )
    print(f"{'-' * 80}")
    ...
    for result in suite_results:
        ...
        print(
            f"{name:<25} {count:>8} {f1:>8.3f} {precision:>10.3f} {recall:>8.3f} ${cost:>9.4f}"
        )
...
def main():
    ...
    for suite in suites:
        ...
        try:
            ...
            all_suite_results.append({"suite_name": suite["name"], "error": str(e)})
