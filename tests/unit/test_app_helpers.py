...
def test_get_identity_prefers_header_token(app_module, monkeypatch):
    ...
    monkeypatch.setattr(app_module, "extract_token_from_headers", lambda headers: headers.get("cf-access-jwt-assertion"))

    request = SimpleNamespace(cookies={})
    identity = app_module.get_identity(cf_access_jwt_assertion="token-abc", request=request)
...
def test_get_identity_falls_back_to_cookie(app_module, monkeypatch):
    ...
    monkeypatch.setattr(app_module, "verify_cf_access_token", lambda token: {"sub": "cookie-user"})

    request = SimpleNamespace(cookies={"CF_Authorization": "cookie-token"})
    identity = app_module.get_identity(request=request, cf_authorization_cookie="cookie-token")
